  ...
  const elementObserver = new IntersectionObserver((elements) => { 
    ...
    elements.forEach((element) => {
      ...
      if (element.isIntersecting) {
        element.target.style.animation = `${element.target.dataset?.move ?? "move-down"} ${element.target.dataset?.duration ?? 0.5}s ${element.target.dataset?.delay ?? 0}s ease-in-out forwards`;
      } else {

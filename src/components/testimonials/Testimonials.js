...
const Testimonials = () => {
  ...
  useEffect(() => {
    const handleWindowResize = () => {
      ...
      wrapperElement.current.style.transform = `translateX(-${(elementWidth + 30) * clickCount.current}px)`;
    };
    ...
    <TestimonialsContainer>
      ...
      <div className="feedback-container anime" data-move="zoom-out" data-delay={0.2}>
              ...
              localdb.testimonials.map((item, index) => (
                <div className="feedback-wrapper" key={uid()}>
                  ...
                  <div className="user-info-container">
                    {/* USER AVATAR */}
                    <img src={item.userImage} alt="user-avatar" className="user-avatar" />
        ...
        <div className="feedback-bottom-container">
          <button className="left-button" onClick={handlePrevButton}><ArrowBackIcon /></button>
          <button className="right-button" onClick={handleNextButton}><ArrowForwardIcon /></button>
        </div>
      </div>
    </TestimonialsContainer>
  );
